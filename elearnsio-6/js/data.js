...
function dLog(){
  ...
  var list=$("llist");
  if(!S.logs.length){list.innerHTML='<div class="empty"><div class="ei"><i data-lucide="book-open" style="width:22px;height:22px;stroke:currentColor"></i></div><div style="font-size:14px;font-weight:600;color:var(--t2)">No entries</div></div>';return}
  var h="";S.logs.slice(0,30).forEach(function(e,idx){
    var n=(e.tod||"day")==="night",bg=n?"var(--ig)":"var(--accg)",col=n?"var(--i)":"var(--acc)",ico=n?"<i data-lucide="moon" style="width:15px;height:15px;stroke:currentColor"></i>":"<i data-lucide="sun" style="width:15px;height:15px;stroke:currentColor"></i>";
...
function dRt(){
  ...
  var h="";S.routes.forEach(function(r,i){var ds="";try{ds=new Date(r.ts).toLocaleDateString("en-AU",{day:"numeric",month:"short",year:"numeric"})}catch(e){}
    h+='<div class="card" data-ri="'+i+'" style="position:relative"><div class="cr"><div class="ci" style="background:var(--accg);color:var(--acc)"><i data-lucide="route" style="width:18px;height:18px;stroke:var(--acc)"></i></div><div class="cb"><div class="ct">'+r.distance.toFixed(1)+' km · '+Math.round(r.duration)+' min</div><div class="cs">'+ds+' · '+(r.destType==="loop"?"Loop":"P2P")+' · '+r.radius+'km radius</div></div><button class="bg rt-del" data-di="'+i+'" style="color:var(--d);font-size:14px;padding:8px" title="Delete">🗑</button></div></div>'});
  list.innerHTML=h;
