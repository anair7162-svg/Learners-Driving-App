  ...
  acTimer=setTimeout(function(){
    ...
    .then(function(r){return r.json()}).then(function(results){
      ...
      results.forEach(function(r){
        ...
        d.innerHTML='<span class="ac-icon"><i data-lucide="map-pin" style="width:15px;height:15px;stroke:var(--acc)"></i></span><div style="flex:1;min-width:0"><div class="ac-text">'+short+'</div><div class="ac-sub">'+type+'</div></div>';
...
function showPinnedBanner(label,lat,lng){
  ...
  banner.innerHTML=
    '<div style="font-size:18px;flex-shrink:0"><i data-lucide="map-pin" style="width:15px;height:15px;stroke:var(--acc)"></i></div>'+
...
function showWaypointSheet(){
  ...
  var h='<h3><i data-lucide="map-pin" style="width:16px;height:16px;stroke:var(--i)"></i> Waypoints <span class="pill pill-i">'+S.waypoints.length+'</span></h3>';
  ...
  h+='<button class="btn ba" id="wp-gen"><i data-lucide="zap" style="width:15px;height:15px;stroke:var(--bg0)"></i> Build Loop Route</button>';
  h+='<button class="btn bs" id="wp-clr" style="flex:none;padding:12px 14px">✕</button>';
  h+='</div>';
  h+='<div class="wp-tap-hint"><i data-lucide="lightbulb" style="width:12px;height:12px;stroke:var(--t3)"></i> Tap the map to add a waypoint</div>';
  $("sh").innerHTML=h;
...
function renderStopUI(){
  ...
  h+='<div class="add-stop-bar'+(S.addingStop?" active":"")+'" id="add-stop-btn"><i data-lucide="plus-circle" style="width:14px;height:14px;stroke:var(--w)"></i> Add waypoint to route</div>';
  if(S.addingStop){
      ...
      stTimer=setTimeout(function(){
        ...
        .then(function(r){return r.json()}).then(function(results){
          ...
          results.forEach(function(r){
            var d=document.createElement("div");d.className="ac-item";
            var short=r.display_name.split(",").slice(0,3).join(",");
            d.innerHTML='<span class="ac-icon"><i data-lucide="map-pin" style="width:15px;height:15px;stroke:var(--acc)"></i></span><span class="ac-text">'+short+'</span>';
            d.addEventListener("click",function(){
              S.addingStop=false;
