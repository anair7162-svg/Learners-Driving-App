...
$("sinp").addEventListener("input",function(){
  ...
  acTimer=setTimeout(function(){
    ...
    .then(function(r){return r.json()}).then(function(results){
      var drop=$("ac-drop");
      if(!results.length){drop.innerHTML='<div class="ac-item"><span class="ac-icon">x</span><span class="ac-text">No results</span></div>';drop.classList.add("on");return}
      drop.innerHTML="";
...
function showPinnedBanner(label,lat,lng){
  ...
  banner.innerHTML=
    ...
    '<button class="bg" id="mban-x" style="font-size:20px;padding:4px 10px;color:var(--t3);flex-shrink:0">x</button>';
...
function showWaypointSheet(){
  ...
  h+='<button class="btn bs" id="wp-clr" style="flex:none;padding:12px 14px">x</button>';
  h+='</div>';
...
function renderWaypointList(){
  ...
  S.waypoints.forEach(function(w,i){
    h+='<div class="stop-item">'+
      ...
      '<button class="stop-del wp-del" data-wi="'+i+'">x</button>'+
    '</div>';
  });
  h+='</div>';
  return h;
    ...
    sinp.addEventListener("input",function(){
      ...
      stTimer=setTimeout(function(){
        ...
        .then(function(r){return r.json()}).then(function(results){
          ...
          if(!results.length){box.innerHTML='<div class="ac-item"><span class="ac-icon">x</span><span class="ac-text">No results</span></div>';return}
          box.innerHTML="";
          results.forEach(function(r){
            var d=document.createElement("div");d.className="ac-item";
            var short=r.display_name.split(",").slice(0,3).join(",");
