...
function dRt(){
  ...
  var h="";S.routes.forEach(function(r,i){var ds="";try{ds=new Date(r.ts).toLocaleDateString("en-AU",{day:"numeric",month:"short",year:"numeric"})}catch(e){}
    h+='<div class="card" data-ri="'+i+'" style="position:relative"><div class="cr"><div class="ci" style="background:var(--accg);color:var(--acc)"></div><div class="cb"><div class="ct">'+r.distance.toFixed(1)+' km · '+Math.round(r.duration)+' min</div><div class="cs">'+ds+' · '+(r.destType==="loop"?"Loop":"P2P")+' · '+r.radius+'km radius</div></div><button class="bg rt-del" data-di="'+i+'" style="color:var(--d);font-size:14px;padding:8px" title="Delete">x</button></div></div>'});
  list.innerHTML=h;
