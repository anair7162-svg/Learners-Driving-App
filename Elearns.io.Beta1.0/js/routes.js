...
var POI_MAP = {
  "parking":      {q: '["amenity"="parking"]',          icon: "\uD83C\uDD7F\uFE0F", css: "poi-parking",    label: "Parking Lot"},
  "roundabouts":  {q: '["junction"="roundabout"]',      icon: "\uD83D\uDD04",       css: "poi-roundabout", label: "Roundabout"},
  "hills":        {q: '["natural"="peak"]',             icon: "\u26F0\uFE0F",       css: "poi-hill",       label: "Hill / Incline"},
...
function fetchPOIs(rd) {
  ...
  var s = bounds.getSouth() - .008, bw = bounds.getWest() - .008;
  var n = bounds.getNorth() + .008, e = bounds.getEast() + .008;
  var bbox = s + "," + bw + "," + n + "," + e;
  var queries = "";
  ...
  fetch("https://overpass-api.de/api/interpreter?data=" +
    encodeURIComponent("[out:json][timeout:12];(" + queries + ");out center 60;"))
    .then(function(r) { return r.json(); })
    .then(function(data) {
...
function renderPOIs(rd, elements) {
  ...
  var routePts = rd.geometry.coordinates.filter(function(c, i) { return i % 12 === 0; })
  ...
  elements.forEach(function(el) {
    ...
    var pm = POI_MAP[poiType];
    var name = tags.name || pm.label;
    if (onRoute) onC++; else nearC++;
    var div = document.createElement("div");
    div.className = "poi-mk " + pm.css;
    if (!(W.prefs[poiType] > 0)) div.style.opacity = "0.45";
    ...
    popHtml += '<div style="font-weight:700;font-size:13px;margin-bottom:4px">' + pm.icon + " " + name + '</div>';
    popHtml += '<div style="font-size:11px;color:#888;margin-bottom:2px">' + pm.label + '</div>';
    popHtml += '<div style="font-size:11px;font-weight:600;color:' + (onRoute ? "#22d89e" : "#4d9eff") + ';margin-bottom:8px">';
    popHtml += onRoute ? "On your route" : "~" + Math.round(minDist) + "m away";
    popHtml += '</div>';
    if (!onRoute) {
      popHtml += '<button onclick="rerouteViaPOI(' + lat + ',' + lon + ')" ';
      popHtml += 'style="width:100%;padding:8px;border-radius:8px;background:var(--acc);color:#060a12;border:none;font-weight:700;font-size:12px;font-family:var(--f);cursor:pointer">';
      popHtml += 'Route via here</button>';
    }
    popHtml += '</div>';
...
window.rerouteViaPOI = function(lat, lon) {
  ...
  osrm([insertLL, poiPt, insertLL]).then(function(detour) {
    ...
    showResult();
    toast("Rerouted via landmark \u2014 +" + Math.round(detour.duration / 60) + " min");
  });
};
...
function showResult() {
  ...
  h += '<button class="btn bs" id="rprev" style="flex:none;padding:12px 16px">\u25CB Preview</button>';
