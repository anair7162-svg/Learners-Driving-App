...
function searchWithFallback(query, limit, cb) {
  fetch(buildSearchUrl(query, limit))
  ...
  acTimer = setTimeout(function() {
    searchWithFallback(q, 8, function(results) {
      ...
      results.forEach(function(r) {
        ...
        var road = addr.road || "";
        var suburb = addr.suburb || addr.town || addr.city || "";
        var state = addr.state || "";
        var postcode = addr.postcode || "";
        var mainLine = "";
        if (houseNum && road) mainLine = houseNum + " " + road;
        else if (road) mainLine = road;
        else mainLine = r.display_name.split(",").slice(0, 2).join(",").trim();
        var subParts = [suburb, state, postcode].filter(Boolean);
        ...
        var fullDisplay = r.display_name;
        d.innerHTML = '<span class="ac-icon">\u25CB</span>' +
          '<div style="flex:1;min-width:0">' +
          '<div class="ac-text">' + mainLine + '</div>' +
          '<div class="ac-sub">' + subLine + '</div>' +
          '</div>';
        d.addEventListener("click", function() {
          $("sinp").value = mainLine;
          drop.classList.remove("on");
          pinLocation(parseFloat(r.lat), parseFloat(r.lon), fullDisplay);
        });
        drop.appendChild(d);
      });
      drop.classList.add("on");
...
function doDestSearch() {
  ...
  searchWithFallback(q, 3, function(results) {
    ...
    results = sortByProximity(results);
    pinLocation(parseFloat(results[0].lat), parseFloat(results[0].lon), results[0].display_name);
  });
}
...
function showWaypointSheet() {
  ...
  h += '<p style="font-size:12px;color:var(--t2);margin-bottom:10px;line-height:1.5">';
...
$("lbtn").addEventListener("click", function() {
  ...
  navigator.geolocation.getCurrentPosition(onPos, function() {
    navigator.geolocation.getCurrentPosition(onPos, function() {
      if (!done) { done = true; toast("Could not get location \u2014 check browser permissions"); }
    ...
    sinp.addEventListener("input", function() {
      ...
      stTimer = setTimeout(function() {
        searchWithFallback(q, 5, function(results) {
          ...
          if (!results.length) {
            box.innerHTML = '<div class="ac-item"><span class="ac-icon">\u2013</span><span class="ac-text">No results</span></div>';
            return;
          }
          results = sortByProximity(results);
          box.innerHTML = "";
