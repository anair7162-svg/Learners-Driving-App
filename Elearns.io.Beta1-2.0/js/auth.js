...
tryRestoreSession(function(ok) {
  if (ok) {
    ...
    enterApp();
    setTimeout(function() {
      toast("\u26A0 You're not logged in \u2014 progress won't be saved");
    }, 1500);
  }
});
...
function requestLocation() {
  ...
  navigator.geolocation.getCurrentPosition(function(p) {
    ...
    if (err.code === 1) {
      toast("Location permission needed \u2014 trying again...");
      navigator.geolocation.getCurrentPosition(function(p) {
        S.userLL = {lat: p.coords.latitude, lng: p.coords.longitude};
        setStartFromGPS();
      }, function() {
        toast("Using approximate location \u2014 enable GPS in settings for accuracy");
        ipFallbackLocation();
      }, {enableHighAccuracy: false, timeout: 10000, maximumAge: 300000});
    } else {
      toast("GPS timed out \u2014 retrying...");
      navigator.geolocation.getCurrentPosition(function(p) {
        S.userLL = {lat: p.coords.latitude, lng: p.coords.longitude};
        setStartFromGPS();
      }, function() {
...
function ipFallbackLocation() {
  fetch("https://ipapi.co/json/")
    .then(function(r) { return r.json(); })
    .then(function(d) {
      if (d && d.latitude && d.longitude) {
        S.userLL = {lat: d.latitude, lng: d.longitude};
        setStartFromGPS();
      } else {
        fallbackLocation();
      }
    })
    .catch(function() { fallbackLocation(); });
}

function fallbackLocation() {
  toast("Search a destination to get started");
